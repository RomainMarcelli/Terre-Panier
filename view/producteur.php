<!DOCTYPE html>
<html lang="fr">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Détails du Producteur</title>
    <link rel="shortcut icon" href="../../img/favicon.ico" />
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
    <link rel="stylesheet" href="../../style.css">
    <link rel="stylesheet" href="../../css/all.css">
    <link rel="stylesheet" href="/mds-back/TerreOPanier/css/producteurs.css">
    <link rel="stylesheet" href="../../css/responsive.css">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Bungee+Shade&family=Swanky+and+Moo+Moo&display=swap" rel="stylesheet">
</head>

<body>
    <header>
        <nav class="flex">
            <div class="logo_burger">
                <div id="mySidenav" class="sidenav">
                    <a id="closeBtn" href="#" class="close">×</a>
                    <ul>
                        <li class="icon_burger"><a href="./myAccount/favoris.php"><img src="../../img/favoris.svg" alt=""></a></li>
                        <li class="icon_burger"><a href="./profile.php"><img src="../../img/profile.svg" alt=""></a></li>
                        <li class="icon_burger"><a href="./myAccount/panier.php"><img src="../../img/panier.svg" alt=""></a></li>
                        <li><a href="./productPages/fruit.php">Fruits & Légumes</a></li>
                        <li><a href="./productPages/viandes.php">Viande & Charcuteries</a></li>
                        <li><a href="./productPages/poissons.php">Poisson & Fruits de mer</a></li>
                        <li><a href="./productPages/epicerie.php">Epicerie</a></li>
                        <li><a href="./productPages/cremerie.php">Crémerie</a></li>
                        <li><a href="./productPages/boisson.php">Boissons</a></li>
                        <li><a href="">Bons Plans</a></li>
                        <li><a href="../all_ateliers.php">Ateliers</a></li>
                        <li><a href="./producteur.php">Producteurs</a></li>
                    </ul>
                </div>

                <a href="#" id="openBtn">
                    <span class="burger-icon">
                        <span></span>
                        <span></span>
                        <span></span>
                    </span>
                </a>
                <a href="../index.php" id="logo"><img id="logo_img" src="../../img/Logo final V2 SVG.svg" alt=""></a>
            </div>
            <div class="flex right-nav">
                <a href=".//myAccount/favoris.php"><img src="../../img/favoris.svg" alt=""></a>
                <a href="./profile.php"><img src="../../img/profile.svg" alt=""></a>
                <a href="./myAccount/panier.php"><img src="../../img/panier.svg" alt=""></a>
            </div>
        </nav>

        <div class="flex" id="menu">
            <a href="./productPages/fruit.php">Fruits & Légumes</a>
            <a href="./productPages/viandes.php">Viande & Charcuteries</a>
            <a href="./productPages/poissons.php">Poisson & Fruits de mer</a>
            <a href="./productPages/epicerie.php">Epicerie</a>
            <a href="./productPages/cremerie.php">Crémerie</a>
            <a href="./productPages/boisson.php">Boissons</a>
            <a href="">Bons Plans</a>
            <a href="../all_ateliers.php">Ateliers</a>
            <a href="./producteur.php">Producteurs</a>
        </div>
    </header>
    <div class="search-bar">
        <input type="text" placeholder="Rechercher...">
        <a href="#" class="search-icon">
            <i class="fa fa-search"></i>
        </a>
    </div>
    <h2>À LA RENCONTRE DES PRODUCTEURS</h2>
    <div class="producteurs">
        <div class="img_producteur">
            <img src="../img/Bertrand - Le Verger de l'Azur.jpg" alt="Producteurs">
        </div>
        <div class="details">
            <div class="fav">
                <img src="../../img/favoris.svg" alt="">
            </div>
            <div class="title">
                <h3>
                    Bertrand - Le Verger de l'Azur
                </h3>
                <p>Agricultueur</p>
            </div>

            <div class="lieu">
                <svg width="34" height="61" viewBox="0 0 34 61" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                    <rect width="34" height="61" fill="url(#pattern0_490_849)" />
                    <defs>
                        <pattern id="pattern0_490_849" patternContentUnits="objectBoundingBox" width="1" height="1">
                            <use xlink:href="#image0_490_849" transform="matrix(0.015625 0 0 0.00870902 0 -0.00512295)" />
                        </pattern>
                        <image id="image0_490_849" width="64" height="116" xlink:href="data:image/png;base64," />
                    </defs>
                </svg>
                <p>Draguignan (Provence-Alpes-Côte d’Azur)</p>
            </div>

            <div class="avis_client">
                <p>Avis des clients : 4,5</p>
            </div>
            <div class="histoire">
                <p class="gras">Son histoire</p>
                <p>Depuis 1985, Bertrand cultive avec passion des produits de qualité, sains et savoureux en plein cœur de la Provence. Partant d'une petite parcelle, il a su faire grandir son exploitation en respectant les traditions agricoles locales. Aujourd'hui, sa ferme propose une variété de fruits, légumes mais aussi de vins à déguster au plus vite !</p>
            </div>
        </div>
    </div>
    <h2>SA PRODUCTION</h2>
    <div id="produits" class="grid-container">
        <div class="produit">
            <div>
                <img src="../img/pamplemousse.jpg" alt="Pamplemousse">
            </div>
            <div class="description-produit">
                <p class="title">Le Verger de l’Azur <br>Provence-Alpes-Côte d’Azur</p>
                <p class="type">Pamplemousse Rose</p>
                <div class="btn_price">
                    <p class="poids">1kg <br> <span>3,80€/kg</span></p>
                    <button>+</button>
                    <p class="price">3,80€</p>
                </div>
            </div>
        </div>
        <div class="produit">
            <div>
                <img src="../img/melon.jpg" alt="Melon">
            </div>
            <div class="description-produit">
                <p class="title">Le Verger de l’Azur <br>Provence-Alpes-Côte d’Azur</p>
                <p class="type">Melon Bio Charantais</p>
                <div class="btn_price">
                    <p class="poids">Unité <br> <span>750g</span></p>
                    <button>+</button>
                    <p class="price">4,20€</p>
                </div>
            </div>
        </div>
        <div class="produit">
            <div>
                <img src="../img/citron.jpg" alt="Citron">
            </div>
            <div class="description-produit">
                <p class="title">Le Verger de l’Azur <br>Provence-Alpes-Côte d’Azur</p>
                <p class="type">Citron de Menton</p>
                <div class="btn_price">
                    <p class="poids">500g <br> <span>5,98€/kg</span></p>
                    <button>+</button>
                    <p class="price">2,99€</p>
                </div>
            </div>
        </div>
        <div class="produit">
            <div>
                <img src="../img/aubergine.jpg" alt="Aubergine">
            </div>
            <div class="description-produit">
                <p class="title">Le Verger de l’Azur <br>Provence-Alpes-Côte d’Azur</p>
                <p class="type">Aubergine Violette Bio</p>
                <div class="btn_price">
                    <p class="poids">1kg <br> <span>5,50€/kg</span></p>
                    <button>+</button>
                    <p class="price">5,50€</p>
                </div>
            </div>
        </div>
        <div class="produit">
            <div>
                <img src="../img/laitue.jpg" alt="Citron">
            </div>
            <div class="description-produit">
                <p class="title">Le Verger de l’Azur <br>Provence-Alpes-Côte d’Azur</p>
                <p class="type">Salade Laitue</p>
                <div class="btn_price">
                    <p class="poids">unité <br> <span>100g</span></p>
                    <button>+</button>
                    <p class="price">2,30€</p>
                </div>
            </div>
        </div>
        <div class="produit">
            <div>
                <img src="../img/pates_riz.jpg" alt="Pates_riz">
            </div>
            <div class="description-produit">
                <p class="title">Le Verger de l’Azur <br>Provence-Alpes-Côte d’Azur</p>
                <p class="type">Pâtes Risoni Nature</p>
                <div class="btn_price">
                    <p class="poids">1kg <br> <span>7,60€/kg</span></p>
                    <button>+</button>
                    <p class="price">7,60€</p>
                </div>
            </div>
        </div>
    </div>
    <footer>
        <div class="links_reseaux">
            <div id="footer">
                <div class="flex links-section">
                    <div class="links">
                        <p>Nous contacter</p>
                        <p>01.00.00.00.00</p>
                        <p>Terreôpanier@gmail.com</p>
                        <p>40 Rue du Chemin Vert - 75011 Paris</p>
                    </div>
                    <div class="links">
                        <p>Le service client</p>
                        <a href="">Livraisons</a>
                        <a href="">Remboursements</a>
                        <a href="">Commandes</a>
                    </div>
                    <div class="links">
                        <p>Notre société</p>
                        <a href="">Nos engagements</a>
                        <a href="">L'équipe</a>
                        <a href="">Nous rejoindre</a>
                    </div>
                </div>
            </div>
            <h6>Suivez-nous sur les réseaux</h6>
            <div class="flex reseaux">
                <span><a href="https://www.facebook.com/people/Terre-Ô-Panier/61557170597906/" target="_blank"><img src="../img/facebook_f_logo_icon_145290.png" alt=""></a></span>
                <span><a href="https://www.instagram.com/terreopanier?igsh=a2Y3OHc5OHk1ZGEx" target="_blank"><img src="../img/b9f2a640b168cdb9f865185facee4cd3.png" alt=""></a></span>
            </div>
        </div>
        <div id="mentions" class="flex">
            <a href="">Conditions générales de vente</a>
            <a href="">Informations légales</a>
            <a href="">Données personnelles</a>
            <a href="">Politique de cookies</a>
        </div>
    </footer>
    <script src="../script.js"></script>
</body>

</html>